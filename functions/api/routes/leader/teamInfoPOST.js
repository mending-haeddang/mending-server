...
module.exports = async (req, res) => {
  ...
  const { name, memberCount, userID } = req.body // teamID, userID 가 여기 들어가는게 맞나?
  
  // 필요한 값이 없을 때 보내주는 response
  if (!name || !memberCount) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
  ...
  try {
    ...
    const teamID = 1;
    const teamCode ='123456'
    const team = await teamDB.addTeam(client, memberCount, teamCode);   // 팀코드 임의로 해당 파일 내에서 구현함
    const teamInfo = await teamDB.userInfo(client, userID);
    
    // 성공적으로 users를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_ALL_USERS_SUCCESS, {team, teamInfo}));
