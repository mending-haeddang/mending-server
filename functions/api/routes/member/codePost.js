...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const team = await teamDB.getTeamIdByTeamCode(client, teamCode);

    const user = await userDB.addUser(client, name, team.id);
    const questions = await questionDB.getAllQuestions(client)
    // 성공적으로 users를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_ALL_USERS_SUCCESS, {user, questions}));
